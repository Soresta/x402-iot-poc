...
**Status (2026-09-13):** #2 half observed (the ledger records the stop). #6: key
live, one buying run left. #7 ran as PARTIAL, not 24 h. #1, #3 not run. #4 and #5
wait on decisions and on strangers.
...
> refused. Still needed: the `.env` line (step 1) and step 3. The soak has ended,
> so step 3 can run now.
